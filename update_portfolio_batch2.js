...
const results = JSON.parse(fs.readFileSync('analysis_results_batch2.json', 'utf8'));

// 1. Move and rename images
results.forEach(item => {
    const oldPath = path.join('/app/applet/public/images', item.originalFile);
    ...
    return `    {
      ...
      fallback: (
        <div className="w-full h-full bg-gradient-to-br from-zinc-900 to-zinc-800 flex flex-col justify-center items-center p-6 relative">
          ...
          <span className="text-zinc-400 text-xs font-bold text-center">${item.title}</span>
        </div>
      )
    }`;
}).join(',\n');
